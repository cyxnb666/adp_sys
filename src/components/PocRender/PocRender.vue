  ...
  render(createElement) {
    ...
    if ((this.node && this.node.nodeType === NODE_TYPE.COMPARISON) || [NODE_TYPE.FIELD].includes(this.node.nodeType)) {
      ...
      if (this.node.methodField) {
        ...
        >{this.node.text}<i slot='suffix' onClick={(e) => { this.$emit('methodParamIconClick', { arrItemIndex: this.arrItemIndex, node: this.node, index: this.index, type: 'method_FIELD', path: this.path, i: this.i, paramId: this.paramId }) }} class='el-input__icon el-icon-circle-plus-outline' /></span>
      } else {
        return <span
          class={`mr-5 ${this.node.nodeType === NODE_TYPE.FIELD ? 'field' : ''}`}
          onClick={(e) => { this.$emit('clickTemplate', { node: this.node, event: e, index: this.index }) }}
          onContextmenu={(e) => { this.$emit('rightTemplate', { node: this.node, event: e, index: this.index }) }}
        >{this.node.text ? this.node.text : this.node.label}<i slot='suffix' onClick={(e) => { this.$emit('plusIconRightclick', { arrItemIndex: this.arrItemIndex, node: this.node, index: this.index, type: 'FIELD', path: this.path, i: this.i, paramId: this.paramId }) }} class='el-input__icon el-icon-circle-plus-outline' /></span>
      }
    }
    // 此处渲染 方法的参数中的字段
    // NESTING 渲染嵌套节点.
      ...
      return createElement(
        ...
        this.node.arr.map((item, index) => {
          ...
          if (item && item.nodeType === NODE_TYPE.FIELD) {
            ...
            >{item.label}<i slot='suffix' onClick={(e) => { this.$emit('plusIconRightclick', { node: this.node, i: index, index: this.index, type: 'FIELD', path: this.path }) }} class='el-input__icon el-icon-circle-plus-outline' /></span>
          }
          ...
          if ([NODE_TYPE.FIELD].includes(item.nodeType)) {
            if (item && item.exprNode) {
              ...
              return <span
                ...
                onContextmenu={(e) => { this.$emit('rightTemplate', { node: this.node, event: e, index: this.index, i: index, insertBracket: 'insertBracket', path: this.path }) }}
              >{item.label}<i slot='suffix' onClick={(e) => { this.$emit('TREEEXplusIconClick', { event: e, node: this.node, i: index, index: this.index, type: 'FIELD', path: this.path }) }} class='el-input__icon el-icon-circle-plus-outline' /></span>
            }
          }
          // 对于表达式类型的子节点，也进行渲染，根据表达式类型不同，创建不同类型的子组件 Vinput 进行渲染
          if (item && item.nodeType === NODE_TYPE.EXPRESSION) {
