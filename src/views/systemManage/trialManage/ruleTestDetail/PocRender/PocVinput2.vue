<template>
  <div class="relative treeInput" :class="{ 'express-node': paramId }">
    ...
    <template v-if="vInputValue.dataType === 'method' || vInputValue.nodeType === 'METHOD'">
      <RenderTemplate :tree-data="treeData" :i="i" :path="path" :index="index" :node="vInputValue"/>
    </template>
    <template v-else-if="vInputValue.dataType === 'field' || vInputValue.nodeType === 'FIELD'">
      <RenderTemplate :param-id="paramId" :i="i" :path="path" :index="index" :tree-data="treeData" :node="vInputValue" />
    </template>
    <template v-else-if="format.type === 'input'&&format.desc==='<布尔参数>'">
      <span style="width: 90%;">
        <!-- <el-select v-model="vInputValue.paramValue" placeholder="请选择" disabled>
          <el-option
        ...
        <span>{{ vInputValue.paramValue }}</span>
      </span>
    </template>
    <template v-else-if="format.type === 'input'&&format.desc!=='<布尔参数>'">
    ...
    <template v-else-if="format.type === 'string'">
      <!-- :type="format.returnType === 'string' ? (vInputValueLength>30?'textarea':'text') : 'number'" -->
      <!-- <span style="display:flex;display: inline-flex;">
        <el-input
          ref="addParamsInput"
          v-model="vInputValue.paramValue"
          class="input"
      ...
      <span class="values">{{ node.paramValueUnit }}</span>
    </template>
    <template v-else-if="format.type === 'int'">
      ...
      <span class="values">{{ node.paramValue }}</span>
    </template>
    <template v-else-if="format.type === 'integer'">
      ...
      <span class="values">{{ vInputValue.paramValue }}</span>
    </template>
    ...
    <template v-else-if="format.type === 'date'">
      ...
      <!-- <span style="width: 80%;display:flex;display: inline-flex;">
        <el-input
          ref="addParamsInput"
          v-model="vInputValue.paramValue"
          class="input"
