<template>
  <div class="rule-edit">
    <el-form ref="form" :model="rule" inline>
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item prop="ruleId" :rules="[
        ...
        <el-col :span="8">
          <el-form-item prop="ruleName" :rules="[
        ...
        <el-col :span="8">
          <el-form-item label-width="140px" prop="ruleTips" :rules="[
        ...
        <el-col :span="8">
          <el-form-item prop="annotation" label="属性标签">
            <el-cascader style="width: 100%" clearable v-model="rule.ruleAttributeTags" :options="groupList"
              :props="{ expandTrigger: 'hover', multiple: true, emitPath: false }"></el-cascader>
          </el-form-item>
        </el-col>
        <el-col :span="16">
          <el-form-item prop="annotation" label="规则说明">
    ...
    <!-- 这是公司产品：规则编辑组件 -->
    <page-rule-editor ref="poc" class="page-rule-editor" :base-u-r-l="baseURL"
      :bom-base-info-id="$store.state.bom.bomBaseInfo" :token="token" :rule-content="rule.ruleContent"
      :code-services="rule.codeServices" :rule-id="rule.ruleId" :module-value="moduleValue"
      @changeEditPage="changeEditPage" @submitRule="submitRule(arguments)" />
  </div>
</template>
  ...
  methods: {
    ...
    saveRuleDefinedEdit(predefineReqModel, submitAnnotation) {
      ...
      saveRuleDefinedEdit({
        ruleDefined: data,
        predefineReqModel,
        ruleMethodDefineModel: [ruleMethodDefineModel]
      }).then(res => {
        if (res.success) {
          this.$message({
