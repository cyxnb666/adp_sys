  ...
  <div class="ruleSetting" @scroll="handleScroll">
        ...
        <ValidationObserver ref="obs" tag="form">
          ...
          >
            ...
            >
              <!-- 决策表设置组件 -->
              <div class="">
                <DecisionTableItem :item="item" :index="index" @modifyDecisionTable="modifyDecisionTable" @addPrefixRule="addPrefixRule" @downloadUserTemplate="downloadUserTemplate" @DeleteDecisionTable="DeleteDecisionTable" @retrieveData="retrieveData">
                  <template #status>
                ...
                <!-- <i v-if="i!==decisionTableList.length-1" class="el-icon-bottom iconArrows mt-10" /> -->
              </div>
              <div class="r-body">
                <div class="flex-wrap">
                  <div class="item">
                  ...
                  <div class="item editBtn" style="float: right;display: block;margin-left: auto;">
                    <!-- <el-button icon="el-icon-edit-outline" type="primary" @click="editRule(item.id,index)">编辑</el-button> -->
                    <el-button
                      type="text"
                      icon="el-icon-edit-outline"
                      :disabled="item.ruleSettingContent || item.approvalStatus === '0'"
                ...
                <div v-show="item.ruleId">
                  ...
                  <div class="rule-preview" style="margin-top: 20px;">
                    <div class="rule-wrap">
                      <div class="rule-view">
                        <span class="value">如果：</span>
                        <span class="value valueContent" style="word-break: break-all;" v-html="item.ruleSettingDesc || '-'" />
                      </div>
                    </div>
                  </div>
                  <!-- <i class="el-icon-error delRuleIcon" @click="delRuleFn(item, index)" /> -->
              ...
              <!-- <div v-if="item.children" style="padding: 20px" class="output-rule">
                ...
                <div class="r-body">
                  ...
                  <div v-if="!showRule" class="ruleContent-wrapper tree-wrapper">
                    <i class="el-icon-error delRuleIcon" />
    ...
    >
      <div class="drawer-wrapper">
        <div class="head">
          {{ $t('decisionTable.decList') }}
        </div>
        <div class="flex-wrap justify-content-between mt-15 mb-15">
          <div>
            <el-button class="btn-border mini" @click="openNewPage('/rule/management')">{{ $t('ruleManage.newdecTable') }}</el-button>
          </div>
          <div>
            <el-input
              v-model="search"
              ...
              clearable
              @change="GetDecisionTableList"
            />
          </div>
        </div>
        <div class="content scroll">
          <NoData v-if="applicationDecisionTableList.length === 0" :text="$t('decisionTable.noDecData')" />
          <div
            v-for="item in applicationDecisionTableList"
            :key="item.decisionTableId"
            class="rule-item "
          >
            ...
            <div class="item flex-wrap align-item-center relative">
              ...
              <span class="value">{{ item.decisionTableDTO.decisionTableName }}</span>
            </div>
            <div class="item  flex-wrap align-item-center">
              <svg-icon icon-class="path" />
              <span class="name">{{ $t('ruleSetting.path') }}：</span>
              <span class="value">/{{ item.decisionTableDTO.path }}</span>
            </div>
            <div v-if="item.ruleDefined">
              <div class="item flex-wrap align-item-center">
                <svg-icon icon-class="content" />
                <span class="name">{{ $t('ruleListItem.ruleContent') }}:</span>
              </div>
              <el-tooltip class="item" effect="dark" placement="left">
                <div slot="content" class="item" style="max-width:500px;font-size:16px" v-html="item.ruleDefined.ruleDesc" />
                <div class="value" style="margin-left: auto" v-html="item.ruleDefined.ruleDesc" />
              </el-tooltip>
            </div>
            <div class="item mt-15 flex-wrap align-item-center justify-content-between">

              <div>
                <el-button type="mini" class="btn-border mini operation" @click="item.showPreviewExcel = true">{{ $t('decisionTable.preview') }}</el-button>
                <el-button type="mini" class="btn-border mini operation" @click="downloadUserTemplate(item)">{{ $t('common.download') }}</el-button>
              </div>
              <div>
                <el-button v-if="!hasTableId(item.decisionTableDTO?.decisionTableId) " type="mini" class="btn-border mini operation" @click="addList(item)">{{ $t('decisionTable.add') }}</el-button>
                <el-button v-else type="mini" class="btn-background mini operation" disabled>{{ $t('decisionTable.added') }}</el-button>
              </div>
            </div>
            <PreviewExcel
              :url="item.url"
              :show-preview-excel.sync="item.showPreviewExcel"
              :title="item.decisionTableDTO.decisionTableName"
              :decision-table-id="item.decisionTableDTO.decisionTableId"
            />
    ...
    <!-- 更新该决策表设置 -->
    <el-dialog :title="currentModel.decisionTableName" :visible.sync="dialogFormVisible">
      <div>
        <h5>
          {{ $t('ruleSetting.effTime') }}-{{ $t('ruleSetting.noEffTime') }}
        </h5>
        <el-date-picker
          v-model="time"
          type="datetimerange"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handlePutDecisionTable">{{ $t('button.OK') }}</el-button>
      </div>
    </el-dialog>
    <el-dialog title="选择规则" class="chooseRuleDialog" :visible.sync="chooseRuleDialog">
      <ruleTree ref="ruleTree" :rule-results-f="RuleResults" :node="{'type':'edge'}" :checked-nodes-f="checkedNodes" :tree-data="treeDataArry" />
      <div slot="footer" class="dialog-footer">
...
import { getMethodSettingDefine, getRuleDefinedDetail } from '@/api/systemManage/rule-manage'
import { getRuleList, getRuleResults, saveRuleDefinedEdit } from '@/api/mainPages/decision'
import { GetDecisionTableList, PostDecisionTableList, GetDecisionTable, getDecisionTableSetting, DeleteDecisionTable, PutDecisionTable, getDecisionTableSettingDetail, FindDecisionTable } from '@/api/mainPages/decision-table'
...
export default {
  ...
  methods: {
    ...
    // 点击dom节点选择规则后保存
    saveChooseRule(ruleId) {
      this.chooseRuleDialog = false
      getRuleDefinedDetail(ruleId).then(async(item) => {
    ...
    retrieveData(index) {
      // this.getQueryRuleSetting1()
      const front = this.ruleSetListForm.ruleSetList[index - 1]
      const last = this.ruleSetListForm.ruleSetList[index]
      this.ruleSetListForm.ruleSetList.splice(index - 1, 2, last, front)

      this.saveRuleList.splice(index - 1, this.saveRuleList[index], this.saveRuleList[index - 1])
    },
    tableItemClick() {
    ...
    GetDecisionTableList() {
      ...
      GetDecisionTableList(data).then((res = []) => {
    ...
    addList(item) {
      const obj = {
        // 决策表数据
        decisionTableId: item.decisionTableDTO.decisionTableId,
        decisionTableSettingId: item.decisionTableDTO.decisionTableSettingId,
        decisionTableTest: item.decisionTableDTO.decisionTableTest,
        decisionUrlId: this.decisionUrlId,
        time: [
          new Date('1970-1-1'),
          new Date('2070-1-1')
        ],
        decisionTableName: item.decisionTableDTO.decisionTableName,
        approvalStatus: item.decisionTableDTO.approvalStatus,
        path: item.decisionTableDTO.path,
        showPreviewExcel: false,
        iseEdit: false,
        url: `${this.$store.state.settings.adp_config.baseURL || process.env.VUE_APP_BASE_API}/export/decisionTable/${item.decisionTableDTO.decisionTableId}`
        // 规则数据
        // ...(item.ruleDefined || {}),
        // ruleSettingDesc: item.ruleDefined?.ruleDesc,
        // ruleSettingContent: item.ruleDefined ? JSON.parse(item.ruleDefined?.ruleContent) : '',
      }
      ...
      if (item.ruleDefined?.ruleId) {
        // 找到要修改元素的位置
        this.ruleSetListForm.ruleSetList.map((val, idx) => {
          if (val.decisionTableId === obj.decisionTableId) {
            this.tableEditIndex = idx
          }
        })
        this.saveChooseRule(item.ruleDefined.ruleId)
      }
    },
