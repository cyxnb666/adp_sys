      ...
      <div class="content">
        ...
        >
          ...
          <el-table-column prop="date" label="审批状态">
            <template slot-scope="scope">
              <div> {{ getApprovalstatus(scope.row.approvalStatus) }} </div>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
<script>
import { PostDeciurl, deleteDecisionTreeInDecURL } from '@/api/mainPages/deci'
import { deleteDecisionTree } from '@/api/mainPages/decisionTree.js'
...
export default {
  ...
  methods: {
    ...
    delBtnFn(row) {
      ...
      this.treeSettingId = row.treeSettingId
    },
    // 删除
    async delPathFn() {
      await deleteDecisionTreeInDecURL({ treeSettingId: this.treeSettingId }).then(res => {
